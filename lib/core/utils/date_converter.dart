...
class DateConverter {
  ...
  static String getTime(int timeStamp, [bool dateOnly = false]) {
    ...
    if (timeDistance.inDays.abs() < 7) {
      return DateFormat("HH:mm, EE").format(time);
    }
    if (timeDistance.inDays.abs() < 365) {
      return DateFormat((dateOnly) ? "dd MMM" : "HH:mm dd MMM").format(time);
    }
  ...
  static String getTimeInAgo(int timeStamp) {
    ...
    if (timeDistance.inMinutes.abs() < 60) {
      return timeDistance.inMinutes.abs().toStringAsFixed(0) + 'mm' + 'ago';
    }
    if (timeDistance.inHours.abs() < 24) {
      return timeDistance.inHours.abs().toStringAsFixed(0) + 'hh' + 'ago';
    }
    if (timeDistance.inDays.abs() < 30) {
      return timeDistance.inDays.abs().toStringAsFixed(0) + 'dd' + 'ago';
    }
    if (timeDistance.inDays.abs() < 365) {
      return (timeDistance.inDays.abs() / 30).toStringAsFixed(0) + 'mm' + 'ago';
    }
    return DateFormat("dd MMM, yyyy").format(time);
  }
  ...
  static List<String> weekDays() {
    return [
      ...
      DateTime(2000, 1, 8, 1),
      DateTime(2000, 1, 9, 1)
    ].map((day) => DateFormat("EEE").format(day)).toList();
  }
}
